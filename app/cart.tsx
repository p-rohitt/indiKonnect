    ...
    <GestureHandlerRootView>
      ...
      {!order && (
        <>
          ...
          style={{height:700}}
            data={products}
            ListHeaderComponent={<View className='flex-row justify-between'>
              
              <Text style={styles.section}>Items</Text>
              <TouchableOpacity className="mt-5 px-5" onPress={()=>clearCart()}>
            ...
            renderItem={({ item }) => (
              <View>
                <View style={styles.row}>
                  ...
                  <Text style={{ flex: 1, fontSize: 18 }}>{`${item.name} (${item.variantWeight}${item.variantUnit})`}</Text>
                  <Text style={{ fontSize: 18 }}>₹{item.price * item.quantity}</Text>
                </View>
                {/* <View className='bg-white'>
                  <Text className='ml-10'>{`Variant: ${item.variantWeight} ${item.variantUnit}`}</Text>
                </View> */}
              </View>
            )}
            ListFooterComponent={
